...
{
	int_htab *htab = sce_paf_malloc(sizeof(int_htab));
	if (!htab) {
		sceClibPrintf("[HTAB] sce_paf_malloc() returned NULL");
		return NULL;
	}
	...
	htab->entries = sce_paf_malloc(htab->size * sizeof(*htab->entries));
	sce_paf_memset(htab->entries, 0, htab->size * sizeof(*htab->entries));
...
void int_htab_free(int_htab *htab)
{
	...
	for (i = 0; i < htab->size; i++) {
		if (htab->entries[i].value != NULL)
			sce_paf_free(htab->entries[i].value);
	}
	sce_paf_free(htab);
}
...
void int_htab_resize(int_htab *htab, unsigned int new_size)
{
	...
	htab->size = new_size;
	htab->used = 0;
	htab->entries = sce_paf_malloc(new_size * sizeof(*htab->entries));
	sce_paf_memset(htab->entries, 0, new_size * sizeof(*htab->entries));
